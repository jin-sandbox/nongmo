...
MongoCache.prototype = {
	execute:function(url,type,arg1,arg2){
		var thiz = this;
		if(thiz.err){
			...
			if(getAll && (fields.body=1) || type == 'getHeaders'){
				collection.findOne({url:url},{fields:fields},
					function(err,result){
						if(result){
							...
							arg1(result.headers,result.body)
						}else{
							arg1();
						}
				...
				collection.updateOne({url:url}, {$set:{headers:arg1,body:arg2}}, {upsert:true,w:1},callback);
			...
			MongoClient.connect(this.dburl, function(err, db) {
				thiz.err = err;
				thiz.db = db;
				thiz.execute(url,type,arg1,arg2);
			});
		}
	},
	close:function(){
	...
	set : function (url, headers, body) {
		this.execute(url,'set',headers,body);
	},
